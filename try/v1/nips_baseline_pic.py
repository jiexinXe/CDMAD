...
parser.add_argument('--baseline-mode', type=str, default='white',
                    choices=['white','black','gray','mean','invmean','symmean','blurmean','patchmean'],
                    help='Scheme-A baseline picture type')
...
# ---------------- Scheme-A Baseline Manager ----------------
class BaselineManager:
    """
    Scheme-A baselines:
      - white/black/gray: constant images in the *current normalized space*
      - mean: EMA dataset mean image estimated online from seen batches
      - invmean: -mean (symmetric around 0)
      - symmean: average logits of mean and -mean
      - blurmean: box-blur(mean)
      - patchmean: patch-wise mean(mean)
    Notes:
      * Works without extra dataset pass (online EMA).
      * If mean not ready yet, falls back to gray.
    """
    def __init__(self, mode: str = "white", ema_m: float = 0.999):
        self.mode = mode
        self.ema_m = float(ema_m)
        self.mean_img = None  # (1,C,H,W) in normalized space

    @torch.no_grad()
    def update(self, imgs: torch.Tensor):
        """imgs: (B,C,H,W) in normalized space."""
        if imgs is None:
            return
        if imgs.dim() != 4:
            return
        # batch mean
        bmean = imgs.mean(dim=0, keepdim=True)  # (1,C,H,W)
        if self.mean_img is None:
            self.mean_img = bmean.detach().clone()
        else:
            # handle resolution change by re-init
            if self.mean_img.shape != bmean.shape:
                self.mean_img = bmean.detach().clone()
            else:
                self.mean_img.mul_(self.ema_m).add_(bmean * (1.0 - self.ema_m))

    @torch.no_grad()
    def _const(self, shape_like: torch.Tensor, val: float):
        B, C, H, W = shape_like.shape
        return torch.full((1, C, H, W), float(val), device=shape_like.device, dtype=shape_like.dtype)

    @torch.no_grad()
    def _box_blur(self, img: torch.Tensor):
        # img: (1,C,H,W)
        _, _, H, W = img.shape
        k = max(3, (min(H, W) // 8) | 1)  # odd kernel
        pad = k // 2
        return F.avg_pool2d(F.pad(img, (pad, pad, pad, pad), mode="reflect"), kernel_size=k, stride=1)

    @torch.no_grad()
    def _patch_mean(self, img: torch.Tensor):
        # img: (1,C,H,W)
        _, _, H, W = img.shape
        p = max(4, min(H, W) // 8)
        # down-up via avgpool
        pooled = F.avg_pool2d(img, kernel_size=p, stride=p, ceil_mode=True)
        up = F.interpolate(pooled, size=(H, W), mode="nearest")
        return up

    @torch.no_grad()
    def get_baseline_images(self, shape_like: torch.Tensor):
        """Return a list of baseline images (each (1,C,H,W)) to evaluate and average logits."""
        mode = self.mode
        if mode == "white":
            return [self._const(shape_like, 1.0)]
        if mode == "black":
            return [self._const(shape_like, -1.0)]
        if mode == "gray":
            return [self._const(shape_like, 0.0)]

        # mean-based
        if self.mean_img is None:
            # warmup fallback
            base = self._const(shape_like, 0.0)
        else:
            base = self.mean_img.to(device=shape_like.device, dtype=shape_like.dtype)
        ...
        if mode == "symmean":
            return [base, -base]
        if mode == "blurmean":
            return [self._box_blur(base)]
        if mode == "patchmean":
            return [self._patch_mean(base)]

        # fallback
        return [self._const(shape_like, 0.0)]

    @torch.no_grad()
    def bias_logits(self, model: nn.Module, shape_like: torch.Tensor):
        """Compute bias logits b = mean_k model(I_k)."""
        imgs = self.get_baseline_images(shape_like)
        logits_list = []
        for img in imgs:
            out = model(img)
...
def main():
    ...
    # Scheme-A baseline manager
    baseline_mgr = BaselineManager(mode=args.baseline_mode, ema_m=0.999)
    ...
    if args.resume:
        ...
        logger.set_names(['bACC(no debias)', 'GM(no debias)', 'bACC(with debias)', 'GM(with debias)', 'Top1(no debias)',
                          'Top1(with debias)'])
    for epoch in range(start_epoch, args.epochs):
        print('\nEpoch: [%d | %d] LR: %f' % (epoch + 1, args.epochs, state['lr']))


        train(labeled_trainloader,unlabeled_trainloader,model, optimizer,ema_optimizer,train_criterion,epoch, baseline_mgr)

        test_acc1, testclassacc1, test_acc2, testclassacc2= validate(test_loader, ema_model,criterion, 'Test Stats ', baseline_mgr)
        GM = 1
    ...
    logger.close()
def train(labeled_trainloader,unlabeled_trainloader, model,optimizer, ema_optimizer, criterion, epoch, baseline_mgr=None):
    if baseline_mgr is None:
        baseline_mgr = BaselineManager(mode=getattr(args,'baseline_mode','white'), ema_m=0.999)
    ...
    for batch_idx in range(args.val_iteration):
        ...
        with torch.no_grad():
            # update online mean baseline using labeled + unlabeled weak images
            baseline_mgr.update(torch.cat([inputs_x, inputs_u], dim=0))
            biaseddegree = baseline_mgr.bias_logits(model, inputs_u)
...
@torch.no_grad()
def validate(valloader, model, criterion, mode, baseline_mgr=None):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    for batch_idx, (inputs, targets, _) in enumerate(valloader):
        ...
        # compute bias logits once using current baseline mode and current resolution
        if biaseddegree is None:
            if baseline_mgr is None:
                baseline_mgr = BaselineManager(mode=getattr(args, 'baseline_mode', 'white'), ema_m=0.999)
            baseline_mgr.update(inputs)
            biaseddegree = baseline_mgr.bias_logits(model, inputs).detach()
        ...
        accperclass += torch.sum(targetsonehot * outputs2onehot, dim=0).cpu().numpy().astype(np.int64)
        accperclass2 += torch.sum(targetsonehot * outputs2onehot2, dim=0).cpu().numpy().astype(np.int64)

        prec1, prec5 = accuracy(outputs, targets, topk=(1, 5))
        prec1d, prec5d = accuracy(outputs2, targets, topk=(1, 5))
        top1.update(prec1.item(), inputs.size(0))
        top5.update(prec5.item(), inputs.size(0))
        top1debias.update(prec1d.item(), inputs.size(0))
        top5debias.update(prec5d.item(), inputs.size(0))

        batch_time.update(time.time() - end)
        end = time.time()

        bar.suffix = '({batch}/{size}) Data: {data:.3f}s | Batch: {bt:.3f}s | Total: {total:} | ETA: {eta:} | '                      'top1: {top1: .4f} | top1(debias): {top1d: .4f}'.format(
            batch=batch_idx + 1,
            size=len(valloader),
            data=data_time.avg,
            bt=batch_time.avg,

...
SPEC_AMP_EMA = None  # torch tensor [1,C,H,Wf] EMA for mep_spec_ema amplitude
...
def forward_probe_logits_batch(model: nn.Module, x_probe_batch: torch.Tensor, probe_mode: str) -> torch.Tensor:
    """Return logits [K, C] for a batch of probe images x_probe_batch [K,C,H,W]."""
    probe_mode = probe_mode.lower()
    if probe_mode == "eval":
        with _temporary_eval(model):
            with torch.inference_mode():
...
def _sample_mep_probes(x_ref: torch.Tensor, dist: str, k: int, antithetic: bool) -> torch.Tensor:
    """Sample K probes from a maximum-entropy null distribution matched to x_ref statistics.
    - dist: 'gauss' or 'unif'
    - antithetic: if True, uses paired samples (mu±eps), requiring even k.
    Returns: x_probes [K,C,H,W] in the same (already preprocessed) input space as x_ref.
    """
    dist = dist.lower()
    B, C, H, W = x_ref.shape
    ch_mean = x_ref.mean(dim=(0,2,3), keepdim=True)   # [1,C,1,1]
    ...
    if antithetic:
        if k % 2 != 0:
            raise ValueError(f"MEP antithetic sampling requires even --mep-k, got {k}")
        k2 = k // 2
    else:
        k2 = k

    if dist in ("gauss","gaussian","normal"):
        eps = torch.randn((k2, C, H, W), device=x_ref.device, dtype=x_ref.dtype)
        if antithetic:
            eps = torch.cat([eps, -eps], dim=0)
        probes = eps * ch_std + ch_mean  # broadcast ch_std/ch_mean
    ...
    if dist in ("unif","uniform"):
        ...
        width = ch_std * math.sqrt(12.0)
        half = 0.5 * width
        # sample in [-0.5, 0.5] then scale to [-half, half]
        u = (torch.rand((k2, C, H, W), device=x_ref.device, dtype=x_ref.dtype) - 0.5) * 2.0
        if antithetic:
            u = torch.cat([u, -u], dim=0)
        probes = u * half + ch_mean
        return probes.contiguous()

    # Fallback: deterministic mean probe replicated K times
    base = ch_mean.expand(k, C, H, W).contiguous()
    return base


# ---------------- MEP-Spec: maximum-entropy in frequency domain (random phase, amplitude matched) ----------------
def _clamp_probes_like(x_ref: torch.Tensor, probes: torch.Tensor, clamp_mode: str) -> torch.Tensor:
    """Clamp probe images to a reasonable value range inferred from x_ref.
    - clamp_mode: 'none' | 'minmax' | 'std3'
    x_ref: [B,C,H,W], probes: [K,C,H,W]
    """
    clamp_mode = (clamp_mode or "minmax").lower()
    if clamp_mode == "none":
        return probes
    ch_mean = x_ref.mean(dim=(0,2,3), keepdim=True)   # [1,C,1,1]
    ch_std  = x_ref.std(dim=(0,2,3), keepdim=True).clamp_min(1e-6)
    if clamp_mode == "std3":
        lo = ch_mean - 3.0 * ch_std
        hi = ch_mean + 3.0 * ch_std
        return torch.clamp(probes, min=lo, max=hi)
    # default: minmax
    ch_min  = x_ref.amin(dim=(0,2,3), keepdim=True)
    ch_max  = x_ref.amax(dim=(0,2,3), keepdim=True)
    return torch.clamp(probes, min=ch_min, max=ch_max)

def _rfft2_amp_mean(x_ref: torch.Tensor, fft_norm: str = "ortho") -> torch.Tensor:
    """Return batch-averaged rFFT amplitude spectrum: [1,C,H,Wf]."""
    X = torch.fft.rfft2(x_ref.float(), dim=(-2,-1), norm=fft_norm)
    amp = torch.abs(X)
    return amp.mean(dim=0, keepdim=True)

def _sample_mep_spec_probes(x_ref: torch.Tensor,
                            k: int,
                            antithetic: bool,
                            amp_mode: str = "batch",
                            amp_ema: float = 0.99,
                            fft_norm: str = "ortho",
                            clamp_mode: str = "minmax") -> torch.Tensor:
    """Sample K random-phase probes using amplitude spectrum matched to x_ref.
    This is a maximum-entropy construction under an amplitude constraint (phase ~ Uniform).
    amp_mode:
      - 'batch': use current batch-averaged amplitude
      - 'ema'  : EMA over batches (global SPEC_AMP_EMA)
    Returns probes: [K,C,H,W]
    """
    global SPEC_AMP_EMA
    amp_mode = (amp_mode or "batch").lower()
    fft_norm = (fft_norm or "ortho").lower()

    B, C, H, W = x_ref.shape
    Wf = W // 2 + 1

    amp_batch = _rfft2_amp_mean(x_ref, fft_norm=fft_norm).detach()  # [1,C,H,Wf]

    if amp_mode == "ema":
        if (SPEC_AMP_EMA is None) or (SPEC_AMP_EMA.shape != amp_batch.shape) or (SPEC_AMP_EMA.device != amp_batch.device):
            SPEC_AMP_EMA = amp_batch.clone()
        else:
            SPEC_AMP_EMA = SPEC_AMP_EMA.mul(amp_ema).add(amp_batch * (1.0 - amp_ema))
        amp_use = SPEC_AMP_EMA
    else:
        amp_use = amp_batch
    ...
    phi = (torch.rand((k2, C, H, Wf), device=x_ref.device, dtype=torch.float32) * (2.0 * math.pi) - math.pi)
    if antithetic:
        phi = torch.cat([phi, -phi], dim=0)  # [K,C,H,Wf]

    # Enforce self-conjugate points to be real (phase = 0) to avoid imaginary leakage at DC/Nyquist
    phi[:, :, 0, 0] = 0.0
    if (W % 2) == 0:  # Nyquist along W exists at index W//2 in rfft axis
        phi[:, :, 0, W // 2] = 0.0
    if (H % 2) == 0:  # Nyquist along H exists at index H//2
        phi[:, :, H // 2, 0] = 0.0
        if (W % 2) == 0:
            phi[:, :, H // 2, W // 2] = 0.0

    # Build rFFT spectrum with fixed amplitude and random phase
    # amp_use: [1,C,H,Wf] -> broadcast to [K,C,H,Wf]
    ampK = amp_use.expand(k, -1, -1, -1)
    spec = ampK * torch.complex(torch.cos(phi), torch.sin(phi))

    probes = torch.fft.irfft2(spec, s=(H, W), dim=(-2, -1), norm=fft_norm).contiguous()  # [K,C,H,W]
    probes = _clamp_probes_like(x_ref, probes, clamp_mode=clamp_mode)
    return probes.to(dtype=x_ref.dtype)

@torch.no_grad()
def get_bias_logits(model: nn.Module,
                    x_ref: torch.Tensor,
                    baseline_picture: str,
                    probe_mode: str,
                    mep_k: int = 4,
                    mep_antithetic: bool = True,
                    mep_eps: float = 1e-6,
                    mep_agg: str = "meanlogit",
                    mep_spec_ema: float = 0.99,
                    mep_spec_clamp: str = "minmax",
                    mep_spec_fft_norm: str = "ortho"):
    """Compute debias logits vector b [C] from a baseline_picture specification.

    - Legacy modes (mean/white/black/...): b = g_theta(I) for a single probe image (point estimate).
    - MEP modes (mep_*): sample K probes and aggregate their responses to estimate a *distributional* baseline.

    Aggregation (mep_agg):
      - 'meanlogit'   : b = mean_k g_theta(probe_k)          (default; closest to CDMAD subtraction)
      - 'logmeanprob' : b = log( mean_k softmax(g_theta(probe_k)) )  (prior-division interpretation)

    Special MEP-Spec modes:
      - 'mep_spec'     : random-phase probes with batch-averaged rFFT amplitude (frequency-domain max-entropy)
      - 'mep_spec_ema' : same, but amplitude is EMA over batches (more stable across iterations)

    Returns:
      (b_logits [C], jsd_avg float) where jsd_avg is the average JSD between each probe prob and the ensemble mean prob
      (0 for single-probe modes).
    """
    mode = baseline_picture.lower()
    mep_agg = (mep_agg or "meanlogit").lower()

    # ---- MEP-Base: distributional baseline (ensemble expectation / mean) ----
    if mode.startswith("mep_"):
        # Choose probe generator
        if mode in ("mep_spec", "mep_spec_ema"):
            amp_mode = "ema" if mode.endswith("_ema") else "batch"
            x_probes = _sample_mep_spec_probes(
                x_ref,
                k=mep_k,
                antithetic=mep_antithetic,
                amp_mode=amp_mode,
                amp_ema=float(mep_spec_ema),
                fft_norm=str(mep_spec_fft_norm),
                clamp_mode=str(mep_spec_clamp),
            )  # [K,C,H,W]
        else:
            dist = mode.split("_", 1)[1]
            x_probes = _sample_mep_probes(x_ref, dist=dist, k=mep_k, antithetic=mep_antithetic)  # [K,C,H,W]

        logits_k = forward_probe_logits_batch(model, x_probes, probe_mode=probe_mode)  # [K,C]
        prob_k = F.softmax(logits_k, dim=1)  # [K,C]
        pi_hat = prob_k.mean(dim=0)  # [C] (unclamped here; clamp only if needed)

        # Aggregate to produce b_logits [C]
        if mep_agg == "logmeanprob":
            b_logits = torch.log(pi_hat.clamp_min(mep_eps))
        else:
            # default: meanlogit
            b_logits = logits_k.mean(dim=0)

        # Average JSD between each probe prob and the mean prob
        jsd_sum = 0.0
        for i in range(prob_k.size(0)):
            jsd_sum += js_divergence(prob_k[i], pi_hat.clamp_min(mep_eps))
        jsd_avg = float(jsd_sum / max(1, prob_k.size(0)))

        return b_logits.detach(), jsd_avg

    # ---- Single probe baseline (point estimate) ----
    x_probe = get_baseline_probe(x_ref, mode)
    b_logits = forward_probe_logits(model, x_probe, probe_mode=probe_mode)
    return b_logits.detach(), 0.0
...
parser.add_argument('--baseline-picture', type=str, default='white',
                choices=['white', 'black', 'gray', 'mean', 'batch_mean',
                         'gauss', 'unif',
                         'mep_gauss', 'mep_unif', 'mep_spec', 'mep_spec_ema'],
                help='Baseline picture / probe for debiasing. '
                     'Use mep_* for MEP-Base ensemble prior estimation.')
parser.add_argument('--probe-ema', type=float, default=0.99,
                    help='EMA factor for probe logits stability tracking (default: 0.99)')

# ---------------- MEP-Base (distributional baseline) ----------------
parser.add_argument('--mep-k', type=int, default=4,
                    help='Number of probes to ensemble for MEP-Base (default: 4).')
parser.add_argument('--mep-eps', type=float, default=1e-6,
                    help='Epsilon clamp for mean probability before log (default: 1e-6).')
parser.add_argument('--mep-antithetic', dest='mep_antithetic', action='store_true',
                    help='Enable antithetic sampling (mu±eps) for MEP probes (default: enabled).')
parser.add_argument('--no-mep-antithetic', dest='mep_antithetic', action='store_false',
                    help='Disable antithetic sampling for MEP probes.')
parser.set_defaults(mep_antithetic=True)


# ---------------- MEP aggregation / MEP-Spec (frequency-domain max-entropy) ----------------
parser.add_argument('--mep-agg', type=str, default='meanlogit', choices=['meanlogit','logmeanprob'],
                    help='How to aggregate K probe responses for MEP baselines: '
                         'meanlogit=mean(logits) (CDMAD-like subtraction), '
                         'logmeanprob=log(mean(softmax)) (prior-division).')

parser.add_argument('--mep-spec-ema', type=float, default=0.99,
                    help='EMA factor for amplitude spectrum in mep_spec_ema (default: 0.99).')
parser.add_argument('--mep-spec-clamp', type=str, default='minmax', choices=['minmax','std3','none'],
                    help='Clamp mode for inverse-FFT probes in mep_spec*: '
                         'minmax=clamp to per-channel [min,max] of x_ref (default), '
                         'std3=clamp to mean±3std, none=no clamp.')
parser.add_argument('--mep-spec-fft-norm', type=str, default='ortho', choices=['ortho','backward'],
                    help='FFT norm for mep_spec* (default: ortho).')

parser.add_argument('--test-probe', type=str, default='fixed', choices=['fixed', 'per_batch'],
                    help='How to compute bias probe during evaluation: '
                         'fixed=compute once from first batch; per_batch=compute each batch.')
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_stats = train(labeled_trainloader,unlabeled_trainloader,model, optimizer,ema_optimizer,train_criterion,epoch)
...
def train(labeled_trainloader,unlabeled_trainloader, model,optimizer, ema_optimizer, criterion, epoch):
    ...
    for batch_idx in range(args.val_iteration):
        ...
        with torch.no_grad():
            ...
            b_logits, probe_jsd_avg = get_bias_logits(
                model, inputs_u, args.baseline_picture, args.probe_mode,
                mep_k=args.mep_k, mep_antithetic=args.mep_antithetic, mep_eps=args.mep_eps,
                mep_agg=args.mep_agg, mep_spec_ema=args.mep_spec_ema,
                mep_spec_clamp=args.mep_spec_clamp, mep_spec_fft_norm=args.mep_spec_fft_norm
            )
...
def validate(valloader,model,criterion,mode):
    ...
    with torch.no_grad():
        ...
        b_logits, _ = get_bias_logits(
            model, x_ref, args.baseline_picture, args.probe_mode,
            mep_k=args.mep_k, mep_antithetic=args.mep_antithetic, mep_eps=args.mep_eps,
                mep_agg=args.mep_agg, mep_spec_ema=args.mep_spec_ema,
                mep_spec_clamp=args.mep_spec_clamp, mep_spec_fft_norm=args.mep_spec_fft_norm
        )
        biaseddegree = b_logits.view(1, -1)
        for batch_idx, (inputs, targets, _) in enumerate(valloader):
            ...
            if args.test_probe == 'per_batch':
                b_logits_batch, _ = get_bias_logits(
                    model, inputs, args.baseline_picture, args.probe_mode,
                    mep_k=args.mep_k, mep_antithetic=args.mep_antithetic, mep_eps=args.mep_eps,
                mep_agg=args.mep_agg, mep_spec_ema=args.mep_spec_ema,
                mep_spec_clamp=args.mep_spec_clamp, mep_spec_fft_norm=args.mep_spec_fft_norm
                )
                biaseddegree = b_logits_batch.view(1, -1)

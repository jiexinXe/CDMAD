...
DIAG_FIELDS = [
    ...
    "zra_energy"] # 用于监控 ZRA-Base 特征能量趋近于 0 的程度

DIAG_F = None
DIAG_WRITER = None
PROBE_EMA = None  # [复用该变量] 现已升级为真实的偏置平滑器
...
def get_single_baseline_probe(x_ref: torch.Tensor, mode: str) -> torch.Tensor:
    mode = mode.lower()
    if mode in ("white", "black", "gray", "gauss", "unif", "uniform", "mean"):
    ...
    if mode == "ema_mean":
        if EMA_MEAN_IMG is None:
            return x_ref.mean(dim=0, keepdim=True).contiguous()
        return EMA_MEAN_IMG.to(device=x_ref.device, dtype=x_ref.dtype).contiguous()
    if mode == "inv_ema_mean":
        if EMA_MEAN_IMG is None:
            return (-x_ref.mean(dim=0, keepdim=True)).contiguous()
        return (-EMA_MEAN_IMG).to(device=x_ref.device, dtype=x_ref.dtype).contiguous()
...
@torch.no_grad()
def get_bias_logits(model: nn.Module, x_ref: torch.Tensor, baseline_mode: str) -> torch.Tensor:
    baseline_mode = baseline_mode.lower()
    if baseline_mode == "sym_ema_mean":
        x_pos = get_single_baseline_probe(x_ref, "ema_mean")
        x_neg = get_single_baseline_probe(x_ref, "inv_ema_mean")
        l_pos = forward_probe_logits(model, x_pos, "eval")
        l_neg = forward_probe_logits(model, x_neg, "eval")
...
def make_probe_like(x_ref: torch.Tensor, kind: str) -> torch.Tensor:
    ...
    if kind in ("const1","ones"): base = torch.ones_like(ch_mean)
    elif kind in ("const0","zeros"): base = torch.zeros_like(ch_mean)
    elif kind in ("const05","half"): base = 0.5 * torch.ones_like(ch_mean)
    elif kind == "mean": base = ch_mean
    elif kind == "zero": base = torch.zeros_like(ch_mean)
    elif kind == "white": base = ch_max
    elif kind == "black": base = ch_min
    elif kind == "gray": base = 0.5 * (ch_min + ch_max)
    elif kind in ("red","green","blue") and C >= 3:
        base = ch_min.clone()
        if kind == "red": base[:,0:1] = ch_max[:,0:1]
        elif kind == "green": base[:,1:2] = ch_max[:,1:2]
        else: base[:,2:3] = ch_max[:,2:3]
    else:
        base = ch_mean
...
parser.add_argument('--probe-mode', type=str, default='eval', choices=['eval'], help='Always eval to avoid BN running-stats writeback.')

parser.add_argument('--baseline-picture', type=str, default='zra_base',
                choices=['white', 'black', 'gray', 'mean', 'batch_mean', 'fma_base', 'bpa_base', 'zra_base'],
                help='Single baseline picture for debiasing. Try "zra_base" for Zero-Response alignment.')
parser.add_argument('--probe-ema', type=float, default=0.99, help='EMA factor for probe logits stability tracking (default: 0.99)')
parser.add_argument('--log-bn-drift', action='store_true', help='Log BN running-stats drift.')
...
def train(labeled_trainloader,unlabeled_trainloader, model,optimizer, ema_optimizer, criterion, epoch):
    ...
    for batch_idx in range(args.val_iteration):
        ...
        bn_drift = 0.0
        zra_energy_now = 0.0
        global X_BASE, OPT_BASE, PROBE_EMA
        
        with torch.no_grad():
            outputs_u_raw, features_u = model(inputs_u)
            update_ema_mean_image(inputs_u, args.mean_ema_decay)
        ...
        if args.baseline_picture == 'zra_base':
            ...
            with torch.no_grad():
                ...
                b_logits = forward_probe_logits(model, X_BASE, args.probe_mode).detach()
        ...
        # =========================================================================
        # [修改点：偏置实时动量平滑 (EMA Smoothing)]
        # 在这里执行每一轮真正的平滑操作，滤除高频优化噪声
        # =========================================================================
        with torch.no_grad():
            if PROBE_EMA is None:
                PROBE_EMA = b_logits.detach().clone()
            else:
                # 使用 0.99 的动量保证偏置信号的绝对平滑，避免伪标签震荡
            ...
            if epoch > args.debiasstart:
                # [关键！] 使用平滑过后的 PROBE_EMA 进行去偏，而不是用原始波动的 b_logits
                outputs_u_debias = outputs_u_raw - PROBE_EMA.view(1, -1)
        ...
        if args.diag_enable and (batch_idx % args.diag_freq == 0):
            ...
            # 计算当前 b_logits 和平滑后 PROBE_EMA 的距离差异
...
def validate(valloader,model,criterion,mode):
    ...
    with torch.no_grad():
        ...
        if args.baseline_picture == 'zra_base' and X_BASE is not None:
            biaseddegree = forward_probe_logits(model, X_BASE, args.probe_mode).view(1, -1)
        else:

...
parser.add_argument('--baseline-picture', type=str, default='zra_base',
                choices=['white', 'black', 'gray', 'mean', 'batch_mean', 'zra_base'],
                help='Single baseline picture for debiasing. Main method uses zra_base.')
parser.add_argument('--zra-init', type=str, default='mean',
                    choices=['mean', 'white', 'black', 'gray', 'batch_mean', 'random'])
...
def train(labeled_trainloader, unlabeled_trainloader, model, ema_model, optimizer, ema_optimizer, criterion, epoch):
    ...
    for batch_idx in range(args.val_iteration):
        ...
        if args.baseline_picture == 'zra_base':
            ...
            if X_BASE is None:
                # 依然以均值图像为最安全的初始化起点
                if args.zra_init == 'mean':
                    init_probe = EMA_MEAN_IMG.detach().clone() if EMA_MEAN_IMG is not None else inputs_u.mean(dim=0, keepdim=True).detach()
                elif args.zra_init == 'batch_mean':
                    init_probe = inputs_u.mean(dim=0, keepdim=True).detach()
                elif args.zra_init == 'white':
                    init_probe = make_probe_like(inputs_u, 'white').detach()
                elif args.zra_init == 'black':
                    init_probe = make_probe_like(inputs_u, 'black').detach()
                elif args.zra_init == 'gray':
                    init_probe = make_probe_like(inputs_u, 'gray').detach()
                elif args.zra_init == 'random':
                    init_probe = make_probe_like(inputs_u, 'mean').detach() + 0.01 * torch.randn_like(inputs_u[:1])
                else:
                    raise ValueError(f"Unknown zra_init: {args.zra_init}")

                X_BASE = nn.Parameter(init_probe)
                OPT_BASE = optim.Adam([X_BASE], lr=args.fma_lr)
            ...
            with torch.no_grad():
                with _temporary_eval(ema_model):
                    ...
                    if args.baseline_picture == 'zra_base' and X_BASE is not None:
                        b_logits_ema = forward_probe_logits(ema_model, X_BASE.detach(), PROBE_MODE).detach()
                    else:
...
def validate(valloader,model,criterion,mode):
    ...
    with torch.no_grad():
        ...
        if args.baseline_picture == 'zra_base' and X_BASE is not None:
            biaseddegree = forward_probe_logits(model, X_BASE, PROBE_MODE).view(1, -1)
        else:
